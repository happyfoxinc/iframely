...
export function generateDefaultHtmlFromMeta(oembedData) {
    let { title, url, description } = oembedData;

    const html = `<div style="font-family: Arial, sans-serif;border: 1px solid #e5e5e5;border-radius: 3px;box-shadow: 0 1px 2px rgba(0,0,0,0.1);background-color: #fff;height: 213px;display: flex;align-items: center;justify-content: center;">
        <a href="${url}" style="text-decoration: none" target="_blank">
            <div style="padding: 10px;">
                <h3 style="margin: 0 0 10px; font-size: 20px; line-height: 24px; font-weight: bold;">${title}</h3>
                ${description ? `<p style="margin: 0 0 10px;">${description}</p>` : ''}
            </div>
        </a>
    </div>`;
...
export function generateErrorHtml(errorMessage) {
    const html = 
    `<div style="font-family: Arial, sans-serif;border: 1px solid #e5e5e5;border-radius: 3px;box-shadow: 0 1px 2px rgba(0,0,0,0.1);height: 213px;display: flex;align-items: center;justify-content: center;color: red;font-weight: bold;">
        <div style="padding: 10px;">
...
export function getMediaEmbedWrapperHtml(oembedData) {
    const { url, provider_name, html } = oembedData;
    let paddingBottom = '56.25%'; // 16:9 aspect ratio, 9/16 = 0.5625
    let maxWidth;
    ...
    if(!html){
        paddingBottom = '20%'; // 5:4 aspect ratio, 4/5 = 0.2 for cases where we generate the card ourselves
    }

    return `<div ${maxWidth ? `style="max-width: ${maxWidth}"`: ''}><div style="left: 0; width: 100%; height: 0; position: relative; padding-bottom: ${paddingBottom}"><iframe allowfullscreen="" src="${CONFIG.baseAppUrl}/render?url=${url}" style="top: 0; left: 0; width: 100%; height: 100%; position: absolute; border: 0;"></iframe></div></div>`
}